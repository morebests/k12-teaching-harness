...
当前接口支持八年级数学的有限课段与全年蓝图，标准固定使用 CCSS Mathematics。全年蓝图包含单元组织、Narrative、目标分工、实践、时间、关键任务探查与实际单元交接。完整 Lesson 和单元材料、其余四项能力、真实待答恢复、内容历史和生产运行仍由后续票交付。
...
| 全年单元交接 | `GET /v1/threads/{id}/units/{unit_id}?expected_fingerprint=…` | 返回当前实际单元、目标与实践、标准和已采用知识、学校条件、前后联系及探查；包含当前检查状态和两个内容指纹，过期则 409 |
...
```bash
uv run python examples/live_curriculum.py --event 全年蓝图-自己的唯一标识 \
  --request examples/year-request.json --output work/year-result.json
```

[全年请求](../examples/year-request.json) 沿用既有离线实验条件：180×50 分钟，其中 20 课时机动、28 人。`scope=year` 时 `target_codes` 留空，由实际 LC 年级投影完整遍历决定覆盖范围；不能提交目标子集并称为全年。`kind=grade` 内容与原 `kind=section` 分开。单元交接要求当前全年稿指纹，能读草稿但同时返回其状态与检查；下层不能把未通过稿当作已接受版本。当前只读当前版本，不提供历史正文。
...
生成模型只能保存候选。程序核对目标集合、课时预算、引用、渲染与指纹；全年另查父子覆盖、单元职责、数学实践和知识采用的实际身份。隔离审阅调用读取当前实际课程、知识和图件，逐项检查覆盖、数学、进程、条件、证据及来源。阻断问题返回生成模型修订并重查。`completed` 仅表示本次范围的程序与模型检查通过，不表示教师批准、课堂成效、全年逐课材料完备或广泛教学质量达标。
