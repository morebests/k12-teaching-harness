...
01 已完成有限工程闭环，15 已产生全年蓝图及实际单元交接。用户进一步明确：**当前先闭合全年课程规划的方法与质量，暂停向 03／17／06 推进。** 工程完成不能替代全年规划质量成立。按[全年阶段设计](curriculum-progression-workflow-design.md)夯实各阶段成果和 Context，再用[全年与 IM 的评估机制](year-planning-evaluation.md)形成同尺度、可复核的优劣结论；尚未进行新机制的真实运行或正式评分。

02／16 补齐真实回应和草稿路径，04 随任务接入观察，05 提供完整单元及跨层修订所需的主动停止／续作。18 的完整故障矩阵及生产部署后置，不因依赖图上较早可达就成为全年内容生成前置。每项教学产物自己的数学、材料与修订检查仍随交付完成。

原先推进 03 的判断来自对完整 spec、22 张票和拆票记录的 [整体复核](delivery-path-audit.md)，现按用户指令纠正当前优先级，后续范围和硬依赖保留。01 的代表任务不能代替完整 Lesson；当前全年评估也不要求先交付全年逐课材料。文件化与原生运行阶段约定继续有效。
...
| [生成覆盖完整八年级 CCSS 的全年蓝图](issues/15-full-year-blueprint.md) | M/L；工程已有，当前优先整改规划方法与质量 | [从任务接口完成有明确范围的真实课程设计](issues/01-live-curriculum-task.md) | 完整年级 CCSS、全年单元职责和 Narrative、实际单元交接；新增阶段检查与同尺度评估待实现 |
