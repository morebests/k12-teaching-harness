...
1. 当前入口是 01。之后全年蓝图（15）和单课材料（03）是教学主线，在 17 汇合为连续三课。02、16 提供真实回应与草稿路径；04、05、18 逐步加入观察、取消和恢复。这些一起组成首个教学交付里程碑。
...
本次先保存并推送规划基线，再审阅 [用户交互原型基准](prototype-baseline.md)。这是用户要求的实施顺序，不新增一个通用审批模块，也不改写上述 `Blocked by`。交互基准确认前，产品实现保持未开始；之后每票按其影响核对同一基准，不在每票重复索取同一确认。
