...
更新：2026-09-16。用户已明确当前目标是让全年规划达到 IM 可观察的规划质量，并要求结构化解决方法。当前实施要求见 [规格](spec.md)，交付由 23–26 承接；当前暂停向 03／17／06 推进。下述机制为具体实施设计，尚未运行，不宣称已找到最优 AI 用法或已达到 IM 水平。
...
## 为什么采用这些方法，而不是仅增加提示要求

[规格 Y1–Y6](spec.md#问题价值与验证依据)描述要解决的教学与执行缺口。下面把依据、选择和反证放在一起：真实失败支持寻找改法，并未证明某个技术因素就是原因。

| 待解决的缺口与依据 | 选择及教学价值 | 代价、简单替代与如何判断 |
| --- | --- | --- |
| 标签覆盖和课时合计无法说明完整年度责任 | A／B 单独形成目标含义与布局理由，避免漏项／误读扩散到全部单元 | 多一次交接可能增加开销；若一次充分输入已能形成两份可检验成果，可共享一次调用，仍分别核对前进条件 |
| 目录和宏观叙述无法证明学习怎样累积 | C 提供每单元理解变化与代表经历，D 验证组合后的联系 | 局部 Context 可能丢全局；保留年度视图并精确读取邻接承诺，以未登记依赖和跨单元缺口反例验证 |
| 原审阅没有作者历史仍漏检 | 围绕具体主张取证、计算、独立试做和回修，减少凭总体印象放行 | 新检查也会误报；用合法对照、可驳回反馈和保留样本测检查器，不以增加角色数量证明可靠 |
| 同一长期输入难说明当前判断依靠什么 | 每次围绕问题重新装配有效成果与相关原文，保留诊断记录 | 摘要可能丢信息；原文可回取、装配范围可核对。比较信息遗漏与修订保真，不追求最短 Context |
| 格式、标识及固定读写失败打断设计 | 程序处理固定操作，工具返回可定位结果，让模型集中在教学判断 | 通用工具体系也有维护成本；先扩现有模块的实际缺口，新增工具须说明替代了什么容易出错的工作 |
| 新结果更流畅仍难说明是否更好 | 预先固定判据，独立比较同尺度全年内容，定位弱项后回修 | 专业判断和资料不足会限制结论；明确不可观察、分歧和外部校准需求，不以总分制造确定性 |

Graph 的理由是确保“下一项工作所依靠的成果和检查仍有效”；Agent 的理由是解决尚不能预定查询或试做顺序的问题。直接 LLM 调用适合已有充分依据的设计与判断。实现应由这三种责任决定，不能为了使用某个 API 倒推教学步骤。若额外控制没有减少漏做、过期结果或返工，应合并或撤去；若局部输入造成关键遗漏，应调整装配范围。质量证据决定取舍。
...
1. [检查校准与证据评价（23）](issues/23-year-evaluation-and-check-calibration.md)：先用旧稿、真实错误、新变体和合法对照验证发现、反馈和评分，知道检查器的局限。
2. [目标与布局闭环（24）](issues/24-year-foundation-and-layout.md)：从真实范围产生 A／B，以检查、回修和有效交接验证最小阶段方法；随实际工作实现必需的读写、Context 和图控制。
3. [单元进程与全年核查（25）](issues/25-year-progressions-and-global-checks.md)：用实际 A／B 收口范围，完成所有 C 和 D、教师阅读稿及有效交接。必须有一次从头的真实全年生成，不能只修旧稿。
4. [IM 比较、修订复评与方法证据（26）](issues/26-year-im-comparison-and-revision.md)：固定候选，完成同尺度内容比较，回修弱项；把内容改进和控制方式的效果分别报告。

比较机制保持相同模型、资料可得范围和外部判据：充分输入的现有循环、改善 Context／判据的循环、同资源的显式阶段工作流分别记录。先用可复用实现做小规模试运行，排除不公平条件，再按试运行固定配对重复和未调提示的新情境；不先建设多套完整系统，不同时更换模型和 SDK 后把变化全归因于 Graph。

是否继续全年回修、何时能恢复后续能力，按 [规格的质量门槛](spec.md#验收与质量声明)判断。实现完成、全年内容质量通过、方法获得支持分别记录；仍有关键差距不能以展开 Lesson 替代。本轮只重整规划，代码、新全年稿、检查校准及正式对比分数尚未产生。
