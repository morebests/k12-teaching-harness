...
```mermaid
flowchart TD
    S([START]) --> P["prepare_task｜准备任务与必需依据<br/>程序：规则、知识、来源和累计用量"]
...
4. 达到明确资源上限或外部调用结果未确认：返回 `stopped`；没有形成可审阅候选或无法继续的教学问题返回 `incomplete`。

当前默认不设累计 token 预算，只累计供应商实际用量；已删除按字节预留预算和动态压缩输出的实现。仅调用方明确设置累计阈值时按调用后实际用量判断后续调用，非硬费用上限。已确认的调用缺少计量时标记未知；没有 token 阈值则不因此停止。未确认的调用仍不盲目重放。

作者保存得到 `rendered=false` 时，使用 `render_errors` 中的源字段、公式和原因修订同一 JSON，再保存、重新渲染，成功后送审。程序送审也保留具体错误作为阻断反馈。此处落实上游材料流程的修复循环；“草稿已保留”不是材料完成条件。实际 HTML 与 SVG 仍需走查，渲染成功不能单独证明显示与教学质量。
