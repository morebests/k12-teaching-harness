...
更新：2026-09-16。权威采用记录：[总体架构决定](../issues/06-architecture-conclusion.md)。本页是规格交接入口，详细决定和证据沿下文读取；目标与协作原则仍由 [README](../../../README.md) 定义。
...
上游 Skills 的完整流程与参考继续作为既有四项能力的设计起点和比较基线。没有证据证明另一套机制整体更好，因此不删除它们；独立课程结构和条件性取用已有明确理由。IM 的具体课程已获准用于本轮研究与全年同尺度对照，须记录参考影响；本地 CCSS／LC／学习联系用于设计依据，三者作用保持清楚。
...
| 保留模型自主工作 | 已有充分输入时由模型设计与审查，未知取证时使用局部 Agent；全年按有效阶段成果交接。控制位置服务教学依赖、真实参与、信息隔离和检查适用性；更多控制须有增益证据。见 [执行决定](../issues/03-execution-structure.md) |
...
当前主模型为已实测的 Gemini，经 LangChain Adapter 接入；原生 `create_agent` 可供阶段内动态探索，充分输入的设计／审查可直接调用模型；LangGraph 管理必要交接与检查边界。Claude、OpenAI 和开放权重模型已有接入路径的条件性评估，实际模型、工具和质量分开验证；首版不做自动多模型路由，也不将现有两份分离探针当成已合并系统。[模型取舍及官方依据](model-knowledge-artifact-design.md#模型调用及三类接入)
...
01／15 已有有限真实工程结果。当前用户要求先把全年规划本身做扎实，因为标准标签和宏观叙述未能充分支持课程负责人判断全年学习如何累积；初次审阅仍漏掉实质问题。先展开 Lesson 不能替代这个年度质量责任。

1. 23 校准检查与证据评价，先知道判断依据和漏报／误报。
2. 24 形成完整目标依据、全年主线与布局，检查后交接，避免错误基础扩散。
3. 25 补足全部单元规划进程并核查全年联系、练习、评价和时间；局部结果还需整体对账。
4. 26 固定候选与 IM 同尺度比较，回修弱项并复评；分别验证内容改善、Context 与阶段控制的实际作用。
5. 年度质量具备相应证据后，再沿 03／17／06 及其余能力迁移有效机制，保持各能力自己的教学流程和完成条件；生产与完整运行保证继续按专票验证。

具体要求已进入 [规格正文](../../math-harness-delivery/spec.md)、[工单与路线](../../math-harness-delivery/README.md)和 [全年方法设计](../../math-harness-delivery/curriculum-progression-workflow-design.md)。这些阶段的依据是教学成果之间的依赖，不是为了采用某个框架；集中 Context 和更显式的控制仍是待验证方法。当前沿用文件化、Agent Server dev 和既有 Gemini Adapter，不同步替换传输层以混淆效果。
