...
## 补充：显式图能改善什么，如何检验接近 IM 的内容质量

2026-09-16 补核。这里只复核工作流与 Context 的官方机制，并复用[本轮 IM 官方来源核查](../../math-harness-delivery/comparisons/im-grade8-scope-and-sequence-source-review.md)；未新增模型实验或课堂效果证据。

**显式图有希望改善执行可靠性；课程质量提升需要由输入、教学判断和实际产物证明。** LangGraph 官方把预定代码路径与模型动态选择区分开，并给出“生成—评价—反馈修订”的工作流；这一模式适用于已有成功判据、但需要迭代的任务。它不要求每一步都改成 Agent，也没有证明采用更多节点就能产生更好的课程。[Workflows and agents](https://docs.langchain.com/oss/python/langgraph/workflows-agents#evaluator-optimizer)

对本项目，若将必需读取、当前版本交接、检查与失败路由正确写入代码，就能通过测试证明：所有交付路径都经过指定检查，审阅对象与交付对象一致，失败按约定返回修订。这是应用实现可以验证的控制性质，不是框架对任意业务代码的自动保证。图不能保证模型真正理解所给材料、检查器发现所有数学错误，或教师能照稿教好。把“已调用审阅”升级为“教学质量通过”，仍需独立内容证据。

Context 也不是显式图的独有优势。LangChain 官方支持在 `create_agent` 的模型调用前改变消息与可见工具，并区分本次调用输入和持久历史。因此，补齐规则、装配实际父产物、去除过期全文、给审阅者明确证据要求，也能在现有两个 Agent 中落实。固定阶段应对应必须交接、检查或恢复的实际工作；其相对于改善后的 Agent 的质量与可靠性增益，仍须分别取证。[Context engineering](https://docs.langchain.com/oss/python/langchain/context-engineering)

### 将教学机制比较拆成 A、B、C

针对本次质量问题，将前述有限比较进一步分解如下；原生 SDK 迁移后置，不参加本组。先做同一有限任务的配对，不建设三套完整系统。

| 变体 | 具体变化 | 可以回答的问题 |
| --- | --- | --- |
| A：充分输入的现有能力 | 保留生成与审阅两个 Agent 及现有控制，修正已知契约缺陷，确保同一充分资料与工具可取得；不能故意省掉必需输入。 | 当前简洁循环在公平条件下能做到什么？ |
| B：改善 Context 与运行内判据 | 保留 A 的控制结构，按当前工作装配资料、当前稿和有效反馈；把课程联系、任务目的、真实解法、评价证据等要求落实到审阅输入与结果。 | 改善信息组织和审阅要求，是否已经解决主要内容缺口？ |
| C：同资源的显式阶段化 | 复用 B 的资料、规则、检查器及允许动作；将已知必做动作、版本交接、检查和条件路由交给图，必要探索仍可使用 Agent。 | 强制顺序与交接能否进一步减少漏做、版本错误和无效修订，并保持或提高内容质量？ |

**三组离线评价使用同一套事先冻结的标准**，不能把 B/C 的运行内审阅变严格，再用各自“通过率”证明进步。模型、版本、任务、原稿、允许证据和工具、停止政策保持一致；逐调用记录实际输入、输出、检查、修订及消耗。B 同时改 Context 与判据，只能支持该组合的结论；若要分辨两者，再做一次有限增减。C 若又改变模型、资料范围或教学标准，结果也不能全部归因于 Graph。

优先检查实际产物的数学正确性、全年与单课之间的职责和联系、表征及语言进程、任务是否产生所需学习证据、师生材料是否可用；另计漏做检查、审阅漏报、修订引入的问题和消耗。重复调用或固定样本的程序探针可验证工程控制；稳定的内容优势需要多个可比任务与独立复核，不能由一次成功运行或作者、审阅者的共同自评证明。

### “达到 IM 水平”必须落到可检查的内容

本次 IM 核查能提供具体比较对象：全年数学联系、各层 Narrative、连续几课新增的数学工作、任务与表征、语言支持及检查后的教学应对。建议沿既定 03→17 主线，先让一课材料及连续课段真正体现这些联系，再检查它们能否与全年设计相互校正；每项改图都说明它保障哪项实际工作，并保留产物证据。此处是项目建议，不是 IM 对生成系统的技术要求。

课程内容的条件可比审阅、教师实际备课使用、学生学习结果是三类不同证据。当前只有公开材料与固定生成稿的内容对照；尚不能承诺达到 IM 的教师使用水平，更不能声称学习效果相当。接近目标的依据应是逐步交付和独立检查所得的内容进步，而不是框架名称、节点数或生成篇幅。
