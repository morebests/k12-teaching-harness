...
- **Review Finding:** The student sheet in Candidate v1.2 had 22 KaTeX parse errors due to unescaped underscores inside math text environments. Teacher guides had zero parser errors.
- **Investigation Finding:** Confirmed via real execution of `check_rendering` on `lesson6_7_8_student_materials.md`. KaTeX 0.18.7 reported 22 errors, all stemming from underscore placeholder strings inside display or inline math blocks. Additionally, `probe_tasks_student_and_teacher.md` contained 2 similar unescaped underscore expressions in PROBE-2.
- **Actual Changed Files and Expressions:**
  - `lesson6_7_8_student_materials.md`: Corrected all 22 failing expressions by moving the response blank lines (`________`) completely outside math delimiters:
    1. TASK-6.3 Q1: Rate line converted to Markdown blank outside math: `Rate = ________________________ gallons per minute`
    2. TASK-6.3 Q2: Rate line converted to Markdown blank outside math: `Rate = ________________________ gallons per minute`
    3. TASK-6.3 Q4: Initial volume line converted to: `Initial Volume $b =$ ________________________ gallons`
    4. CFU-6 Q1: Labor rate prompt converted to: `$m =$ ________________________ dollars per hour`
    5. CFU-6 Q2: Fixed fee prompt converted to: `$b =$ ________________________ dollars`
    6. CFU-6 Q3: Equation prompt converted to: `$C =$ ________________________`
    7. CFU-6 Q4: Total charge prompt converted to: `Total Charge = ________________________ dollars`
    8. TASK-7.2 Part 1 Q2: Rate prompt converted to: `$m_A = \frac{\Delta V}{\Delta t} =$ ________________________ gallons per minute`
    9. TASK-7.2 Part 1 Q3: Function equation prompt converted to: `$V_A =$ ________________________`
    10. TASK-7.2 Part 2 Q5: Draining rate prompt converted to: `$m_B = \frac{\Delta V}{\Delta t} =$ ________________________ gallons per minute`
    11. TASK-7.2 Part 2 Q6: Function equation prompt converted to: `$V_B =$ ________________________`
    12. CFU-7 Q1: Rate prompt converted to: `Rate of change $m =$ ________________________`
    13. CFU-7 Q2: Initial volume prompt converted to: `$b =$ ________________________ gallons`
    14. CFU-7 Q3: Equation prompt converted to: `$V =$ ________________________`
    15. TASK-8.2 Part 1 Q3: Climb speed prompt converted to: `$m = \frac{\Delta A}{\Delta t} =$ ________________________ meters per minute`
    16. TASK-8.2 Part 1 Q4: Equation prompt converted to: `$A =$ ________________________`
    17. TASK-8.2 Part 1 Q4: Altitude prediction prompt converted to: `$A(11) =$ ________________________ meters`
    18. TASK-8.2 Part 2 Q3: Fuel rate prompt converted to: `$m = \frac{\Delta F}{\Delta d} =$ ________________________ gallons per mile`
    19. TASK-8.2 Part 2 Q4: Equation prompt converted to: `$F =$ ________________________`
    20. CFU-8 Q3: Snow equation prompt converted to: `$S =$ ________________________`
    21. CFU-8 Q4: Snow prediction prompt converted to: `Depth after 15 hours = ________________________ inches`
    22. PRAC-8 Item 1: Total cost prompt converted to: `Total Cost = ________________________ dollars`
  - `probe_tasks_student_and_teacher.md`: Corrected the 2 expressions in PROBE-2:
    - PROBE-2 Q2: Rate prompt converted to: `Rate of Change $m = \frac{\Delta D}{\Delta t} =$ ________________________ inches per hour`
    - PROBE-2 Q4: Equation prompt converted to: `$D =$ ________________________`
