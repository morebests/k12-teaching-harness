...
| :--- | :--- | :--- |
| Correctly computes $m = 9$, $b = 21$, $C = 9h + 21$, and $\$93$. | Demonstrates strong current procedural and conceptual proficiency on irregular tables and initial value extrapolation. | Advance to Lesson 7 contextual modeling; assign `EXT-6` or peer explanation role. |
| Calculates $m = 9$ correctly, but writes $C = 9h + 57$ (assumes first entry is $b$). | Procedural understanding of rate, but persistent misconception that the first listed table row is always the $y$-intercept. | Provide targeted prompt: *"What is $h$ in the first row? Does $h = 0$? How much did cost grow during the first 4 hours?"* |
| Computes $m = 45$ (subtracts $102 - 57$ but ignores $\Delta h = 9 - 4 = 5$). | Relapsing into the "Delta Y Only" habit from unit tables ($\Delta x = 1$). | Have student use pencil to circle $\Delta C$ in the right column and box $\Delta h$ in the left column, drawing a fraction bar directly connecting circled over boxed. |
| Inverts rate: $m = \frac{5}{45} \approx 0.11$. | Ratio orientation confusion or arithmetic division order slip. | Check whether the student divided $x$ by $y$ intentionally or inverted by accident. Ask: *"Does the technician charge 11 cents per hour, or hours per dollar?"* |
