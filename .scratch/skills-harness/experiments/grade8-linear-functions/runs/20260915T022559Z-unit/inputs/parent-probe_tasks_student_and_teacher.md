# Feasibility Probes: Student Tasks and Teacher Guides

This document delivers complete, usable student-facing task sheets and teacher evaluation guides for the structural feasibility probes used to validate the Grade 8 Curriculum Blueprint.

---

# Section I: Student Task Sheets (Ready for B&W Printing)
...
![Figure 1: Slope Invariance via Similar Triangles](probe1_slope_triangles.svg)
...
1. **Find the Dimensions of Triangle 1 ($\triangle ABD$):**
   - Horizontal leg $AD$: Change in $x$ ($\Delta x_1$) = ____________ units
   - Vertical leg $DB$: Change in $y$ ($\Delta y_1$) = ____________ units
   - Compute the vertical-to-horizontal ratio: $\frac{\Delta y_1}{\Delta x_1} = \frac{\phantom{00000000}}{\phantom{00000000}} =$ ____________

2. **Find the Dimensions of Triangle 2 ($\triangle BCE$):**
   - Horizontal leg $BE$: Change in $x$ ($\Delta x_2$) = ____________ units
   - Vertical leg $EC$: Change in $y$ ($\Delta y_2$) = ____________ units
   - Compute the vertical-to-horizontal ratio: $\frac{\Delta y_2}{\Delta x_2} = \frac{\phantom{00000000}}{\phantom{00000000}} =$ ____________
...
![Figure 2: Water Tank Filling Over Time](probe2_water_tank.svg)
...
2. **Determining the True Rate of Change:**
   - How many hours passed between the first and second measurement? ($\Delta t = \text{______ hours}$)
   - How many inches did the water depth increase during that time? ($\Delta D = \text{______ inches}$)
   - Calculate the true constant rate of change (unit rate) in inches per hour:
     $$\text{Rate of Change } m = \frac{\Delta D}{\Delta t} = \text{__________________}$$
...
3. **Finding the Initial Value ($b$):**
   ...
   - Initial Depth $b =$ ____________ inches.
...
4. **Constructing the Function:**
   - Write an algebraic equation that models the water depth $D$ (in inches) as a linear function of time $t$ (in hours):
     $$D = \text{________________________}$$
...
1. **Constructing the Functions:**
   ...
   - Elena's savings function: $S_E = \text{________________________}$
   - Jaden's savings function: $S_J = \text{________________________}$
...
3. **Finding the Break-Even Point (8.EE.C.8):**
   - Set up an equation to find the number of weeks $w$ when Elena and Jaden will have exactly the same amount of money saved:
     $$\text{________________________} = \text{________________________}$$
   - Solve the equation for $w$. Show every algebraic step.
   - How many weeks will it take? $w =$ ____________ weeks.
   - How much money will each student have at that time? Total Savings = $\$$____________.
