...
def main():
    ...
    if args.upstream:
        ...
        for name, meta in upstream_state["artifacts"].items():
            ...
            state["parent_artifacts"][alias] = {"run": upstream.name, "original_name": name, **meta}
...
"""
    ...
    if args.task in ("repair", "revision"):
        protocol += "\nThe listed parent artifacts are also copied byte-for-byte into your current workspace as version 0. Revise the same filenames to create later versions; leave unaffected files unchanged. Files with upstream- prefixes retain their upstream role. A repair or revision report must identify actual changes and unresolved work.\n"
    if args.task != "blind":
