...
### Continuous Three-Lesson Sequence: Lessons 6, 7, and 8 (Candidate Version 2.0 — Cross-Layer Budget Revision)
...
- **Universal Supports & Accommodations:**
  ...
  - *Visual Scaffold:* A printed two-column difference bracket template is provided directly on the student sheet (`Visual Reference Scaffold: Two-Column Difference Bracket Template`) to help students visually align $\Delta y$ with its corresponding $\Delta x$.
...
### 3. Materials, Practice Structure, and Advance Preparation
...
- **Practice Architecture:** The 50-minute lesson is fully self-contained across Tasks 6.1–6.3, synthesis, and CFU-6. `PRAC-6` problems are designated as selectable/optional practice for station differentiation, review days, or optional take-home study; they are not mandatory gates for core mastery.
- **Advance Preparation:** Estimated 15 minutes. Verify projector display of Table 1 and Table 2. Pre-cut scratch grid paper for students needing extra calculation space.
...
2. Battery discharge:
   - $m = \frac{42 - 74}{14 - 6} = \frac{-32}{8} = -4\text{ percentage points per hour}$.
     *(Note: This represents an absolute decrease of 4% of full capacity per hour, not a 4% proportional decrease of remaining charge).*
...
| Assessment Evidence (`CFU-6`) | Identified Mathematical State (Possible Interpretations) | Confirming Follow-Up & Next Pedagogical Action |
| :--- | :--- | :--- |
| Correctly computes $m = 9$, $b = 21$, $C = 9h + 21$, and $\$93$. | Demonstrates strong current procedural and conceptual proficiency on the sampled irregular table tasks. (Not evidence of permanent mastery across all novel contexts). | Advance to Lesson 7 contextual modeling; provide `EXT-6` challenge or peer discourse role. Monitor retention in subsequent lesson warm-ups. |
| Calculates $m = 9$ correctly, but writes $C = 9h + 57$ (uses the first table entry $57$ as $b$). | *Possible Interpretation:* Student has procedural grasp of rate, but may have assumed the first listed row is the vertical intercept, or copied the first value without checking whether $h = 0$. (A single response does not prove a permanent conceptual deficit). | *Confirming Check & Action:* Ask the diagnostic question: *"What is the value of $h$ in the first row? Does $h = 0$? How much did cost grow during those first 4 hours before the recorded point?"* Guide student to extrapolate backward: $b = 57 - 9(4) = 21$. |
| Computes $m = 45$ (computes $102 - 57$ but does not divide by $\Delta h = 9 - 4 = 5$). | *Possible Interpretation:* Student may be attending only to the change in cost ($\Delta C$), possibly carrying over habits from consecutive tables ($\Delta x = 1$), or may have overlooked the time column during subtraction. | *Confirming Check & Action:* Ask: *"How many hours did it take for the fee to increase by $\$45$? What is the rate per 1 single hour?"* Have student use pencil to circle $\Delta C$ in the output column and box $\Delta h$ in the input column, drawing a fraction bar directly connecting circled over boxed ($\frac{\Delta C}{\Delta h}$). |
| Inverts rate: $m = \frac{5}{45} \approx 0.11$. | *Possible Interpretation:* Ratio orientation confusion (change in input over change in output) or arithmetic order slip when entering division into a calculator. | *Confirming Check & Action:* Check whether division order was intentional or an accidental keying slip. Ask: *"Does the technician charge 11 cents per hour, or does it take 0.11 hours to earn a dollar? Which quantity depends on which?"* Re-anchor with unit rate $m = \frac{\Delta C}{\Delta h}$. |
...
- **Universal Supports & Accommodations:**
  ...
  - *Physical Realia / Concrete Metaphor:* If students struggle with negative slope, refer to an everyday beverage cup: pouring liquid in adds volume (positive rate, rising line); drinking through a straw removes volume (negative rate, falling line).
...
- **Practice Architecture:** Core modeling lab (Tasks 7.1–7.3), constraint discussion, and exit ticket CFU-7 fit within 50 minutes. `PRAC-7` problems provide selectable/optional practice for station rotations, targeted small-group reinforcement, or optional homework.
...
3. Shared Point $(8, 160)$:
   ...
   - Verifications:
     - Tank A: $V_A(8) = 15(8) + 40 = 120 + 40 = 160\text{ gallons}$.
     - Tank B: $V_B(8) = 320 - 20(8) = 320 - 160 = 160\text{ gallons}$. Both evaluate to 160.
...
| :--- | :--- | :--- |
| Correctly determines $m = -24\text{ gal/min}$, $b = 450$, $V = -24t + 450$, and $t = 18.75\text{ min}$. | Demonstrates strong current proficiency in modeling decreasing linear processes, applying negative rates, and solving for domain bounds. | Ready for Lesson 8 graphical scaling; assign extension challenge or peer debrief. |
| Calculates rate as $+24$ (positive) and writes $V = 24t + 450$. | Procedural fluency with quotient, but omitting direction of change in real contexts. | Prompt during warm-up 8: *"If pool starts at 450 and rate is $+24$, does volume increase or decrease? Does the tank fill or drain?"* |
| Writes correct equation, but solves $450 - 24t = 0$ incorrectly (e.g. arithmetic division slip). | Core conceptual understanding is solid, but arithmetic or algebraic sign error when isolating variable. | Clarify two-step equation solving steps: add $24t$ to both sides first ($24t = 450$), then divide. |
...
- **Daily Learning Targets:**
  - *Target 8A:* I can determine the scale of each axis on a coordinate grid and avoid the "counting grid squares as 1 unit" error.
...
- **Universal Supports & Accommodations:**
  ...
  - *Step-by-Step Anchor Chart:* Post in room: 
    ...
    3. *Pick 2 Clean Points:* Write their $(x, y)$ coordinates from axis readings.
    4. *Compute Rate:* Divide $\frac{\text{change in } y}{\text{change in } x}$.

### 3. Materials, Practice Structure, and Advance Preparation
- **Classroom Materials:** Handout `lesson6_7_8_student_materials.md` (Lesson 8), straightedge rulers, pencils, ordinary calculators, teacher projector displaying `task8_1_trap_sketch.svg`, `lesson8_scaled_graphs.svg`, and `cfu8_snow_graph.svg`.
- **Practice Architecture:** The 50-minute lesson covers Launch (Task 8.1), Guided Investigation (Task 8.2), Synthesis, and Exit Ticket CFU-8. `PRAC-8` problems provide selectable/optional practice for station review, homework, or targeted intervention.
- **Advance Preparation:** Estimated 10 minutes. Project Figure 8.0 and Figure 8.1. Check that the projection clearly distinguishes the tick marks on both Graph 8A and Graph 8B.

### 4. Detailed 50-Minute Instructional Timeline
- **Launch / Confronting the Trap (`TASK-8.1`): 8 minutes**
  - Direct class attention to Figure 8.0 (`task8_1_trap_sketch.svg`).
  ...
  - *Key Facilitation Move:* Qualify the rule: Counting boxes as 1 unit each is an error when axes have non-unit scales. If students count grid intervals, they must multiply the number of intervals by the scale value per interval (e.g., $2\text{ boxes} \times 50\text{ mi/box} = 100\text{ mi}$). Calculating coordinate differences directly ($\Delta y / \Delta x$) is the most direct and reliable method.
...
- **Independent Check for Understanding (`CFU-8`): 6 minutes**
  - Individual, silent exit ticket using Figure 8.2 (`cfu8_snow_graph.svg`), collected at the door.
...
#### Anticipated Student Responses (Design Predictions)
1. **Misconception A (Counting Boxes on Graph 8A as Unit Steps):**
...
- Counting grid boxes as 1 unit each ignores the scale values assigned to each box. If vertical boxes count by 50 and horizontal boxes count by 5, then 2 boxes up is $2 \times 50 = 100$ units, and 1 box right is $1 \times 5 = 5$ units. The true rate of change is $\frac{100}{5} = 20\text{ mph}$, which is 10 times larger than the raw box count of 2.
...
2. Initial Value:
   ...
   - Meaning: The drone was at an altitude of 50 meters above the reference ground level when tracking began at $t = 0$ (e.g., launched from an elevated platform/hill, or tracking commenced after it had already ascended to 50 meters).
3. Rate of Change:
   - $\Delta t = 6 - 2 = 4\text{ minutes}$.
   - $\Delta A = 200 - 100 = 100\text{ meters}$.
   - $m = \frac{100\text{ m}}{4\text{ min}} = 25\text{ meters per minute}$.
   - Error check: 4 boxes up represents $4 \times 25 = 100$ meters; 4 boxes right represents $4 \times 1 = 4$ minutes. Counting boxes as 1 unit each gives $\frac{4}{4} = 1$, which ignores the axis scaling.
4. Function Equation:
   - $A = 25t + 50$.
...
1. Initial depth from point $P_0$: Coordinates $(0, 6) \implies b = 6\text{ inches}$.
2. Coordinate Reading & Snowfall Rate:
   - Reading points from Figure 8.2: $P_1 = (4, 16)$, $P_2 = (8, 26)$, (or $P_3 = (12, 36)$).
   - Rate calculation: $m = \frac{16 - 6}{4 - 0} = \frac{10}{4} = 2.5\text{ inches per hour}$ (or $\frac{26 - 16}{8 - 4} = \frac{10}{4} = 2.5\text{ in/hr}$).
3. Equation: $S = 2.5t + 6$.
4. Depth after 15 hours: $S = 2.5(15) + 6 = 37.5 + 6 = 43.5\text{ inches}$.

#### Solutions for `EXT-8`
1. Fuel economy in mpg: Since the truck burns $0.1$ gallons per mile, $1\text{ gallon} \div 0.1\text{ gal/mi} = 10\text{ miles per gallon}$ (reciprocal of rate).
2. Auxiliary tank effect: The line shifts upward parallel to the original line. The slope remains $-0.1$ gal/mi (engine fuel efficiency is unchanged), but the vertical intercept increases from $24$ to $39$ gallons.

#### Solutions for `PRAC-8`
1. T-Shirt Printing:
   - $b = \$75$ (setup fee at $(0, 75)$).
...
| :--- | :--- | :--- |
| Correctly reads points $(0, 6)$ and $(4, 16)$, determines $b = 6\text{ in}$, $m = 2.5\text{ in/hr}$, $S = 2.5t + 6$, and $43.5\text{ in}$. | Demonstrates strong current proficiency in reading non-unit grid scales, extracting coordinates, and constructing linear functions from graphs. | Ready for Lesson 9 (algebraic two-point formulation without a graph). |
| Misreads point coordinates (e.g., reads $P_1$ as $(2, 16)$ or counts grid lines as unit steps). | Student struggles with reading non-unit axis scales independently. | Re-engage with straightedge: physically trace dashed projection lines from point to axes, reading tick mark values. |
| Computes $m = 2.5$ correctly, but writes $S = 2.5t$ (omits $b = 6$). | Treats relationship as proportional ($y = kx$), overlooking the vertical intercept. | Direct attention to $(0, 6)$ on graph: *"At hour 0, was the ground bare or was there already 6 inches of snow?"* |
| Calculation slip in division (e.g., $10/4 = 2.25$). | Minor arithmetic execution error; coordinate reading and formula structure are sound. | Prompt student to re-verify $10 \div 4$ using calculator or long division. |
